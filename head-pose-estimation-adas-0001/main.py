...
def infer(
    ...
    roll: float = model(inputs=[image])[output_layer[0]].squeeze()
    pitch: float = model(inputs=[image])[output_layer[1]].squeeze()
    yaw: float = model(inputs=[image])[output_layer[2]].squeeze()
...
def main():
    ...
    if re.match(r"^image$", args.mode, re.IGNORECASE):
        ...
        r, p, y = infer(model, output_layer, image)
        ...
        while True:
            ...
            if ret:
                ...
                frame = preprocess(frame, W, H)
                r, p, y = infer(model, output_layer, frame)  
                cv2.putText(
                    disp_frame,
                    f"Roll: {r:.2f}",
                    (25, 75),
