...
def main():
    ...
    parser.add_argument("--threshold", "-th", type=float, default=0.2, help="Minimum Confidence Threshold")
    ...
    if re.match(r"^image$", args.mode, re.IGNORECASE):
        ...
        for box in boxes: 
            ...
            elif int(labels[i]) == 2: cv2.rectangle(disp_image, (box[0], box[1]), (box[2], box[3]), (255, 0, 0), 1)  
            elif int(labels[i]) == 3: cv2.rectangle(disp_image, (box[0], box[1]), (box[2], box[3]), (0, 0, 255), 1)  
            i += 1
