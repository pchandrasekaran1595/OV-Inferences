...
def main():
    ...
    parser.add_argument("--filename", "-f", type=str, default="Test_65.jpg", help="Image or Video Filename")
    ...
    if re.match(r"^image$", args.mode, re.IGNORECASE):
        ...
        label_indexes, probs, boxes = infer_boxes(model, output_layer, image, w, h, args.threshold)

        i: int = 0
        for box in boxes: 
            cv2.rectangle(disp_image, (box[0], box[1]), (box[2], box[3]), (0, 255, 0), 1)  
            cv2.putText(disp_image, f"{labels[label_indexes[i]]}", (box[0]-10, box[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 1)     
            i += 1  
        ...
        while True:
            ...
            if ret:
                ...
                label_indexes, probs, boxes = infer_boxes(model, output_layer, frame, w, h, args.threshold)

                i: int = 0
                for box in boxes: 
                    cv2.rectangle(disp_frame, (box[0], box[1]), (box[2], box[3]), (0, 255, 0), 1)  
                    cv2.putText(disp_frame, f"{labels[label_indexes[i]]}", (box[0]-10, box[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 1)     
                    i += 1          
