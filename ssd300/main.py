...
def infer_best_box(
    ...
    probs = result[:, 2]
    best_box = np.argmax(probs)
    label = result[best_box, 1]

    x1 = int(result[best_box][3] * w)
    y1 = int(result[best_box][4] * h)
    x2 = int(result[best_box][5] * w)
    y2 = int(result[best_box][6] * h)

    return int(label), np.max(probs), (x1, y1), (x2, y2)
