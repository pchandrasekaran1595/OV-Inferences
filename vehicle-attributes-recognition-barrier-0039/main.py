...
def setup(target: str) -> tuple:
    ...
    labels: dict = {
        "color" : ["blue", "gray", "yellow", "green", "black", "white", "red"],
...
def main():
    ...
    if re.match(r"^image$", args.mode, re.IGNORECASE):
        ...
        type = labels["type"][np.argmax(model(inputs=[image])[output_layer[0]].squeeze())].title()
        color = labels["color"][np.argmax(model(inputs=[image])[output_layer[1]].squeeze())].title()
