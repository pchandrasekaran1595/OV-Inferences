...
def infer_boxes(
    ...
    result_boxes = model(inputs=[image])[output_layer[0]].squeeze()
    result_labels = model(inputs=[image])[output_layer[1]].squeeze()

    for i in range(result_boxes.shape[0]):
        if result_boxes[i, -1] > threshold:
            labels.append(result_labels[i])
            probs.append(result_boxes[i, -1])
            boxes.append([int(result_boxes[i, 0] * w / W), \
                          int(result_boxes[i, 1] * h / H), \
                          int(result_boxes[i, 2] * w / W), \
                          int(result_boxes[i, 3] * h / H)])
        else: pass
...
def main():
    ...
    if re.match(r"^image$", args.mode, re.IGNORECASE):
        ...
        labels, probs, boxes = infer_boxes(model, output_layer, image, w, h, W, H, args.threshold)
        ...
        while True:
            ...
            if ret:
                ...
                labels, probs, boxes = infer_boxes(model, output_layer, frame, w, h, w, H, args.threshold)
